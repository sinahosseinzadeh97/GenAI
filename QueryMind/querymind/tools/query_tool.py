...
async def fetch_table_schema_info() -> dict[str, dict[str, Any]]:
    ...
    for tr in tables_result.rows:
        ...
        # Check table
        pragma_result = await execute_query(f"PRAGMA table_info('{table_name}')")
        col_info = {}
    ...
    try:
        ...
        if cache is not None:
            ...
            if cached is not None:
                ...
                if memory:
                    memory.add_turn("user", request.nl_query, sql="[cache hit]")
                    memory.add_turn("assistant", f"{cached.row_count} rows returned (cached)", sql=None)
                return cached
        ...
        if not validate_sql(sql):
            if memory:
                memory.add_turn("user", request.nl_query, sql=None)
            return QueryError(
        ...
        if memory:
            memory.add_turn("user", request.nl_query, sql=sql)
            memory.add_turn("assistant", f"{result.row_count} rows returned", sql=None)
