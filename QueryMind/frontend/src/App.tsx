...
import { AlertCircleIcon, BrainCircuitIcon } from "lucide-react";
...
export default function App() {
  ...
  const handleSubmit = async (q: string) => {
    await runQuery(q);
    setRefreshTrigger(prev => prev + 1); // Trigger history refresh after query completes
  };
      ...
      <header className="border-b border-gray-800 bg-gray-900/50 sticky top-0 z-10 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 font-medium tracking-wide">NATURAL LANGUAGE &rarr; SQL</p>
          </div>
        </div>
      </header>
