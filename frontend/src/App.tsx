...
import { AlertCircleIcon, BrainCircuitIcon, DatabaseIcon, FileSearchIcon, BotIcon } from "lucide-react";

export default function App() {
  const { runQuery, status, sql, result, error, loading } = useQueryStream();
  const [refreshTrigger, setRefreshTrigger] = useState(0);
  const [activeMode, setActiveMode] = useState<"sql" | "rag" | "agent">("sql");
      ...
      <header className="border-b border-gray-800 bg-gray-900/50 sticky top-0 z-10 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-400 font-medium tracking-wide">NATURAL LANGUAGE → SQL + RAG + AGENT</p>
          </div>
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                activeMode === "agent" ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-400 hover:text-gray-200"
              }`}
            >
              <BotIcon size={16} />
              Agent Mode
